...
class AfipController:
    ...
    def __init__(self, dataPath):
        f = open('files.json',)
        data = json.load(f)
        dfModels = pd.read_excel(
            data["modelos"][0]["name"], sheet_name=data["modelos"][0]["sheet"], header=0)
        dfSpecialModels = pd.read_excel(
            data["modelos"][1]["name"], sheet_name=data["modelos"][1]["sheet"], header=0)
        dfTiposFacturas = pd.read_excel(
            data["facturas"][0]["name"], sheet_name=data["facturas"][0]["sheet"], header=0)
        dfData = pd.read_csv(
            dataPath+data["archivos"][0]["name"], delimiter="\r\n", header=None, encoding='latin-1', engine='python')
        dfData = dfData.transpose()
        dfOut = pd.DataFrame()
        self.excelReader = ExcelReader(
            dfModels, dfData, dfSpecialModels, dfTiposFacturas, dfOut)
...
class ExcelReader:
    ...
    def normalRow(self, column, dfRow):
        ...
        for col_name in self.dfModels.columns:
            ...
            if col_name == "Tipo de Factura":
                dfRow.insert(self.dfModels.columns.get_loc(
                    col_name), col_name, [factura])
            else:
                outputString = self.dfData[column].str.slice(
                    start=start, stop=end).values[0]
                dfRow.insert(self.dfModels.columns.get_loc(
                    col_name), col_name, [outputString])

        self.rowAdapterFinal(dfRow, valorVenta)
        # self.dfOut = self.dfOut.append(dfRow, ignore_index=True)

        self.dfOut = pd.concat([self.dfOut, dfRow])
    ...
    def specialRow(self, column, dfRow):
        for col_name in self.dfSpecialModels.columns:
            ...
            outputString = self.dfData[column].str.slice(
                start=start, stop=end).values[0]
            dfRow.insert(self.dfSpecialModels.columns.get_loc(
                col_name), col_name, [outputString])
    ...
    def getTipoDeFactura(self, colName):
        outputString = self.dfData[colName].str.slice(
            start=9, stop=12).values[0]
...
class GUI():
    def __init__(self):
        ...
        confirmMessage = "El mes ingresado es " + \
            self.month + ". El año ingresado es " + self.year
        self.confirm = gui.confirm(text=confirmMessage, title='', buttons=[
                                   'Confirmar', 'Cancelar'])
        if self.confirm == "Confirmar" and self.month in self.months and len(self.year) == 4:
            ...
            gui.alert(text='Por favor valide los datos ingresado',
                      title='', button='OK')
